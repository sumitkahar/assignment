	...
	private void header_menu() throws IOException, InterruptedException {
		...
		try{
			...
			for (String menu: menus.split(",")){
				String menu_header = webDriver.findElement(By.xpath(object.getProperty("header_menu"))).getText();
				if(menu_header.contains(menu)){					
					reporter.writeStepResult("Header menu validation", "Expected: " + menu, "Actual list: "+menu_header, "Pass", "Header menu validated", true, null, webDriver);	
				}
				else{
					reporter.writeStepResult("Header menu validation", "Expected: " + menu,"Actual list: "+menu_header,  "Fail", "Header menu validation failed", true, null, webDriver);
				}
			}
			webDriver.findElementByXPath(object.getProperty("header_product")).click();
			if(webDriver.findElementByXPath(object.getProperty("header_product")).getAttribute("class").equals("active")){
				reporter.writeStepResult("Header menu appearance validation", "Expected: " + "Product menu should be active", "Product menu active", "Pass", "Product menu validated for their appearance", true, null, webDriver);
			}
			Thread.sleep(3000);
			List<WebElement> menu = webDriver.findElements(By.xpath(object.getProperty("header_menu")));
			...
			for(int i=2;i<=3;i++){
				...
				reporter.writeStepResult("Header menu appearance validation", "Expected: " + "menu color should be faded", "Actual list: "+"Other menu items are faded", "Pass", "Header menu validated for their appearance", true, null, webDriver);
			}
			
		}
		catch(Exception e){
			e.printStackTrace();
			reporter.writeStepResult("Header menu validation", "Header menu validation", "Validation Failed", "Fail", "Header menu validation failed", true, null, webDriver);
		}
