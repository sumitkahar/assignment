...
public class Contact_Us {
	...
	private void contactUSLink() throws IOException, InterruptedException {
		...
		try{
			...
			webDriver.findElementByXPath(object.getProperty("contact_us")).click();				
			reporter.writeStepResult("Contact US Link validation", "Contact US Link validation", "Expected: " + "Contact US link is displayed at top right corner", "Pass", "Contact US link is validated", true, null, webDriver);
		}
		catch(Exception e){
			e.printStackTrace();
			reporter.writeStepResult("Contact US Link validation", "Contact US Link validation", "Expected: " + "Contact US link is not displayed at top right corner", "Fail", "Contact US link is validated", true, null, webDriver);
		}
	...
	Exception {
		...
		try{
			...
			if(testcase_sr.equals("ContUs_1")){
				contactUSLink();
			}else {
